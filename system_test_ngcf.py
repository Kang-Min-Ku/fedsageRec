...
temp_fd = open("log/fedsage.log","w")
temp_fd.close()
...
file_handler = logging.FileHandler('log/fedsage.log')
...
from dataowner.loader import DataGenerator
from src.util.build_args import py_config_to_args
from src.fed_NGCFPlus import fed_NGCFPlus, train_fedgen
...
def set_up_system():
    ...
    dg = DataGenerator(config.load_data, config.batch_size)
    local_G, local_nodes_ids, _ = dg.generate(config.num_owners, config.global_data_ratio)
    ...
    print("Fed Gen Complete")
    #for owner_i in range(config.num_owners):
    #    print('Fed Local_owners len neighgen - ' + str(len(fed_local_owners[owner_i].edges)))
    fed_ngcf_plus = fed_NGCFPlus(fed_local_owners, test, plain_adj, num_users, num_items)
